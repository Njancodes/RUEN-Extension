...
export function isEncryptedMessage(chatMessage) {
    return chatMessage.textContent.startsWith('ENC:');
}

export async function decryptMessage(chatMessage, inversekey) {
    try {
...
export async function decryptChatMessages(inversekey) {
...
export function waitForElm(selector, callback) {
...
export function setupSendButton() {
...
// Initialize only when running in a real browser (not during tests)
