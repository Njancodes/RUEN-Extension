(() => {
      ...
      (function(global, factory2) {
        ...
        function create() {
          ...
          function findType(typeName) {
            ...
            const name11 = typeName.toLowerCase();
            let otherName;
            for (otherName of typeList) {
              if (otherName.toLowerCase() === name11) {
          ...
          function findTypeNames(value) {
            const matches = typeList.filter((name11) => {
              const type = typeMap.get(name11);
          ...
          function findSignature(fn, signature, options) {
            ...
            if (exact) {
              remainingSignatures = [];
              let name11;
              for (name11 in fn.signatures) {
                remainingSignatures.push(fn._typedFunctionData.signatureMap.get(name11));
              }
            } else {
          ...
          function mergeExpectedParams(signatures, index) {
            ...
            signatures.forEach((signature) => {
              ...
              let name11;
              for (name11 of paramSet) {
                typeSet.add(name11);
              }
            });
            return typeSet.has("any") ? ["any"] : Array.from(typeSet);
          }
          function createError(name11, args, signatures) {
            let err, expected;
            const _name = name11 || "unnamed";
          ...
          function conflicting(params1, params2) {
            ...
            for (let i = 0; i < ii; i++) {
              ...
              let overlap = false;
              let name11;
              for (name11 of typeSet2) {
                if (typeSet1.has(name11)) {
                  overlap = true;
                  break;
                }
              }
          ...
          function createTypedFunction(name11, rawSignaturesMap) {
            typed3.createCount++;
            ...
            const generic = function generic2() {
              ...
              return typed3.onMismatch(name11, arguments, signatures);
            };
          ...
          function _onMismatch(name11, args, signatures) {
            throw createError(name11, args, signatures);
          }
          ...
          typed3 = function(maybeName) {
            ...
            for (let i = start; i < arguments.length; ++i) {
              ...
              if (!named) {
                name11 = checkName(name11, thisName);
              }
              mergeSignatures(allSignatures, theseSignatures);
            }
            return createTypedFunction(name11 || "", allSignatures);
          };
  ...
  function factory(name11, dependencies12, create, meta) {
    function assertAndCreate(scope) {
      var deps = pickShallow(scope, dependencies12.map(stripOptionalNotation));
      assertDependencies(name11, dependencies12, scope);
      return create(deps);
    }
    assertAndCreate.isFactory = true;
    assertAndCreate.fn = name11;
    assertAndCreate.dependencies = dependencies12.slice().sort();
    if (meta) {
      assertAndCreate.meta = meta;
    }
    return assertAndCreate;
  }
  function assertDependencies(name11, dependencies12, scope) {
    var allDefined = dependencies12.filter((dependency) => !isOptionalDependency(dependency)).every((dependency) => scope[dependency] !== void 0);
    if (!allDefined) {
      var missingDependencies = dependencies12.filter((dependency) => scope[dependency] === void 0);
      throw new Error('Cannot create function "'.concat(name11, '", ') + "some dependencies are missing: ".concat(missingDependencies.map((d) => '"'.concat(d, '"')).join(", "), "."));
    }
  }
  ...
  var createTyped = /* @__PURE__ */ factory("typed", dependencies, function createTyped2(_ref) {
    ...
    typed3.onMismatch = (name11, args, signatures) => {
      var usualError = typed3.createError(name11, args, signatures);
      ...
      signatures.some((sig) => !sig.params.includes(","))) {
        var err = new TypeError("Function '".concat(name11, "' doesn't apply to matrices. To call it ") + "elementwise on a matrix 'M', try 'map(M, ".concat(name11, ")'."));
  ...
  function optimizeCallback(callback, array, name11, isUnary) {
      ...
      if (numberOfArguments >= 1 && numberOfArguments <= 3) {
        return {
          ...
          fn: function fn() {
            ...
            return _tryFunctionWithArgs(fastCallback, args.slice(0, numberOfArguments), name11, callback.name);
          }
        };
      }
      return {
        ...
        fn: function fn() {
          ...
          return _tryFunctionWithArgs(fastCallback, args, name11, callback.name);
        }
      };
    }
    if (isUnary === void 0) {
  ...
  var createClone = /* @__PURE__ */ factory(name6, dependencies7, (_ref) => {
    var {
      typed: typed3
    } = _ref;
    return typed3(name6, {
  ...
  var name10 = "pickRandom";
  var dependencies11 = ["typed", "config", "?on"];
  var createPickRandom = /* @__PURE__ */ factory(name10, dependencies11, (_ref) => {
    var {
      typed: typed3,
      config: config4,
      on
  ...
  var Rubiks3Cube = class {
    ...
    #moveR() {
      ...
      let cback = clone4(this.#back);
      let cdown = clone4(this.#down);
      let cup = clone4(this.#up);
      let cfront = clone4(this.#front);
      for (let i = 0; i < 3; i++) {
    ...
    #moveD() {
      ...
      let cback = clone4(this.#back);
      let cright = clone4(this.#right);
      let cleft = clone4(this.#left);
      let cfront = clone4(this.#front);
      for (let i = 0; i < 3; i++) {
